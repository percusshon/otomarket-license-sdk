...
> **エンジニアでない方へ**: ライセンスキーの発行だけなら、**コードは一切不要**です。OtoMarket の商品設定で「ライセンスキーを発行」を有効にするだけで、購入者ごとにキーが自動発行されます。このリポジトリは、製品に認証（台数制限・期限・失効）を組み込みたい**開発者向け**です。概要から読みたい方は **[ライセンス連携の概要](docs/license-integration/ja/overview.md)** と **[用語集（身近な例で解説）](docs/license-integration/ja/glossary.md)** をどうぞ。開発は外注やAIアシスタント任せでも大丈夫です（ガイドを渡せば組み込みコードを生成できます）。
>
> **Not an engineer?** Issuing license keys requires **no code at all** — enable "Issue license key" in your OtoMarket product settings and every buyer gets a key automatically. This repository is for **developers** who want to build activation (device limits, expiry, revocation) into a product. To get oriented, start with the **[integration overview](docs/license-integration/en/overview.md)** and the **[glossary](docs/license-integration/en/glossary.md)**. You can also hand these guides to a contractor or an AI assistant to write the integration for you.
