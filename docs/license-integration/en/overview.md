...
> New here? Read the **[glossary](./glossary.md)** first. Every term
> is explained with a familiar example.
...
```mermaid
flowchart TD
    A["Creator: create the product<br/>get its productKey"] --> B{"How much<br/>licensing?"}
    B -->|"Proof of purchase only"| C["Lv0: keys only<br/>no code, auto-issued"]
    B -->|"Enforce it<br/>(devices/expiry/revoke)"| D{"Which language?"}
    D -->|"C++/JUCE"| E["Drop in the C++ SDK"]
    D -->|"JS/TS"| F["Drop in the JS/TS SDK"]
    D -->|"Other languages"| G["Call the REST API directly"]
    E --> H["activate / verify / deactivate<br/>OtoMarket returns an<br/>Ed25519-signed license"]
    F --> H
    G --> H
    H --> I["Product: verify the signature<br/>with the public key (works offline)<br/>gate features on the result"]
    I --> J["Pre-publication test:<br/>draft product + test keys verify<br/>the real binary → publish as-is"]
```

Each stage maps to a guide below ([quickstart](./quickstart.md) → [SDK integration](./integration.md) / [REST API](./rest-api.md) → the "in-production testing" section inside the integration guide). New to the terms? See the [glossary](./glossary.md).
